...
MgrUser.initColumn = function () {
    var columns = [
        ...
        {title: '主岗', field: 'majorpositionName', align: 'center', valign: 'middle', sortable: true},
        {title: '职务', field: 'rankName', align: 'center', valign: 'middle', sortable: true},
 ...
 * @param
 */
MgrUser.roleUnified = function () {
    if (this.check()) {
    // var res = this.issetApp();
    // if(!res){
    //     Feng.error("请给用户分配统一应用~!")
    //     return;
    // }
        ...
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/mgr/role_unified/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
...
MgrUser.setApp = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/mgr/role_setapp/' + this.seItem.id
        });
        this.layerIndex = index;
    }
};
 ...
 * 检查是否分配子应用
 */
MgrUser.issetApp = function () {
    var res = "";
    if (this.check()) {
        var userId = this.seItem.id;
        var ajax = new $ax(Feng.ctxPath + "/mgr/issetApp", function (data) {
            res = data;
        });
