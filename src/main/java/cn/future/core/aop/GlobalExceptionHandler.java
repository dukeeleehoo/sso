...
public class GlobalExceptionHandler {
    ...
    public String unAuth(AuthenticationException e) {
        log.error("用户未登陆：", e);
        if(UserAgentUtils.isMobileOrTablet(getRequest())){
    ...
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public String usernamepasswordempty(UserNameAndPasswordException e, Model model) {
        model.addAttribute("tips", "请输入用户名和密码!");
        if(UserAgentUtils.isMobileOrTablet(getRequest())){
            return "/mobile/login.html";
        }
        return "/login.html";
