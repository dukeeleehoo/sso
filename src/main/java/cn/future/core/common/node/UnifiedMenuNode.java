 ...
 */
package cn.future.core.common.node;
 ...
 */
public class UnifiedMenuNode implements Comparable {
     ...
     * 子节点的集合
     */
    private List<UnifiedMenuNode> children;
    /**
     * 子系统唯一标识
     */
     ...
     * 查询子节点时候的临时集合
     */
    private List<UnifiedMenuNode> linkedList = new ArrayList<UnifiedMenuNode>();

    public UnifiedMenuNode() {
        super();
    }

    public UnifiedMenuNode(Long id, Long parentId) {
        super();
        this.id = id;
    ...
    public UnifiedMenuNode setUnqiuecode(String unqiuecode) {
        this.unqiuecode = unqiuecode;
        return this;
    }

    public static UnifiedMenuNode createRoot() {
        return new UnifiedMenuNode(0L, -1L);
    }
    ...
    public List<UnifiedMenuNode> getChildren() {
        return children;
    }

    public void setChildren(List<UnifiedMenuNode> children) {
    ...
    public int compareTo(Object o) {
        UnifiedMenuNode menuNode = (UnifiedMenuNode) o;
     ...
     * 构建页面菜单列表
     */
    public static List<UnifiedMenuNode> buildTitle(List<UnifiedMenuNode> nodes) {
        if (nodes.size() <= 0) {
            return nodes;
        }
        //剔除非菜单
     ...
     * @return
     */
    private static List<UnifiedMenuNode> mergeList(List<UnifiedMenuNode> menuList, int rank, Map<Long, List<UnifiedMenuNode>> listMap) {
        //保存当次调用总共合并了多少元素
        int n;
        //保存当次调用总共合并出来的list
        Map<Long, List<UnifiedMenuNode>> currentMap = new HashMap<>();
