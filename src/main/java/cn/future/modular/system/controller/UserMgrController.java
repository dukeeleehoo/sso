...
import cn.future.modular.system.factory.UserFactory;
import cn.future.modular.system.model.User;
import cn.future.modular.system.service.*;
import cn.future.modular.system.warpper.UserWarpper;
...
import cn.future.modular.system.transfer.UserDto;
...
public class UserMgrController extends BaseController {
    ...
    public String addView( Model model) {
        List<Rank> ranks = rankService.findList();
        model.addAttribute("ranks", ranks);
        List<Majorposition> majorpositions = majorpositionService.findList();
        model.addAttribute("majorpositions", majorpositions);
     ...
     */
//    @Permission
    @RequestMapping("/role_unified/{userId}")
    public String roleUnified(@PathVariable Integer userId, Model model) {
        if (ToolUtil.isEmpty(userId)) {
            throw new ServiceException(BizExceptionEnum.REQUEST_NULL);
        }
        User user = this.userService.selectOne(new EntityWrapper<User>().eq("id", userId));
        model.addAttribute("userId", userId);
        model.addAttribute("userAccount", user.getAccount());
        return PREFIX + "user_roleunified.html";
    }
     ...
     */
//    @Permission
    @RequestMapping("/role_setapp/{userId}")
    public String setApp(@PathVariable Integer userId, Model model) {
        if (ToolUtil.isEmpty(userId)) {
            throw new ServiceException(BizExceptionEnum.REQUEST_NULL);
        }
        User user = this.userService.selectOne(new EntityWrapper<User>().eq("id", userId));
    ...
    public Object changePwd(@RequestParam String oldPwd, @RequestParam String newPwd, @RequestParam String rePwd) {
        ...
        String oldMd5 = Md5Encrypt.md5(oldPwd).toLowerCase();
        if (user.getPassword().equals(oldMd5)) {
            String newMd5 = Md5Encrypt.md5(newPwd).toLowerCase();
    ...
    @ResponseBody
    public ResponseData add(@Valid UserDto user, BindingResult result,String check_val) {
        if (result.hasErrors()) {
            throw new ServiceException(BizExceptionEnum.REQUEST_NULL);
        }
        user.setApplicationids(check_val);
        // 判断账号是否重复
        ...
        if(mUser != null){
            throw new ServiceException(BizExceptionEnum.USER_SHOUJI_REG);
        }
        // 完善账号信息
        ...
        //调用推送接口。
        pushService.pushUser(user1,"add");
        return SUCCESS_TIP;
    }
    ...
    @ResponseBody
    public ResponseData edit(@Valid UserDto user, BindingResult result,String check_val) throws NoPermissionException {
        if (result.hasErrors()) {
            throw new ServiceException(BizExceptionEnum.REQUEST_NULL);
        }
        User oldUser = userService.selectById(user.getId());
        //手机号码校验
        if (ToolUtil.isNotEmpty(user.getMobile())) {
            User mUser = userService.getByMobile(user.getMobile());
            if (mUser != null) {
        ...
        if (ShiroKit.hasRole(Const.ADMIN_NAME)) {
            User newUser = UserFactory.editUser(user, oldUser);
            this.userService.updateById(newUser);
            //调用推送接口。
            pushService.pushUser(newUser,"update");
            return SUCCESS_TIP;
        } else {
            ...
            if (shiroUser.getId().equals(user.getId())) {
                ...
                //调用推送接口。
                pushService.pushUser(user1,"update");
                return SUCCESS_TIP;
            } else {
    ...
    public ResponseData delete(@RequestParam Integer userId) {
        ...
        //调用推送接口。
        pushService.pushUser(userService.selectById(userId),"delete");
        return SUCCESS_TIP;
    }
    ...
    public ResponseData reset(@RequestParam Integer userId) {
        ...
        User user = this.userService.selectById(userId);
        user.setSalt(ShiroKit.getRandomSalt(5));
        user.setPassword(Md5Encrypt.md5(Const.DEFAULT_PWD).toLowerCase());
     ...
     * 分配统一角色
     */
    @RequestMapping("/setUnifiedRole")
    @BussinessLog(value = "分配角色", key = "userId,roleIds", dict = UserDict.class)
    @ResponseBody
    public ResponseData setUnifiedRole(@RequestParam("userId") Integer userId, @RequestParam("roleIds") String roleIds) {
...
//        }
        ...
        assertAuth(userId);
        this.unifiedUserRoleService.setRoles(userId, roleIds,user.getSubsystemadmin());
        return SUCCESS_TIP;
    }
    ...
    @ResponseBody
    @Permission(Const.ADMIN_NAME)
    public ResponseData setApp(@RequestParam("userId") Integer userId, @RequestParam("appid") String appid) {
        User user = userService.selectById(userId);
        Application application = applicationService.selectById(appid);
        user.setSubsystemadmin(application.getAppid());
        user.updateById();
        return SUCCESS_TIP;
    }
     ...
     * 检查用户是否分配应用
     */
    @RequestMapping("/issetApp")
    @ResponseBody
    public Boolean issetApp(@RequestParam("userId") Integer userId) {
        User user = userService.selectById(userId);
        if(StringUtils.isNotBlank(user.getSubsystemadmin())){
            return true;
        }else{
            return false;
        }
